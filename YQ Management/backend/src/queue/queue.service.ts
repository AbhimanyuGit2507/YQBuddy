...
export class QueueService {
  ...
  async createQueue(tenantId: string, name: string, formConfig?: any) {
    const queue = await this.prisma.queue.create({
      data: { tenantId, name, status: QueueStatus.ACTIVE, formConfig },
    });
  ...
  async updateQueue(queueId: string, data: { name?: string, formConfig?: any, nextQueueId?: string | null, allowAppointments?: boolean, requireManualCheckIn?: boolean, appointmentGranularityMins?: number }) {
  ...
  async joinQueue(queueId: string, customerName: string, phone: string | null, isAppointment = false) {
    ...
    if (queue) {
      this.webhooksService.triggerWebhooks(queue.tenantId, 'TOKEN_JOINED', token);
    }
    return token;
  }
  ...
  async advanceTurn(queueId: string) {
    ...
    const queue = await this.prisma.queue.findUnique({ where: { id: queueId }});
    if (queue) {
      this.webhooksService.triggerWebhooks(queue.tenantId, 'TOKEN_SERVING', token);
    }
    return token;
  }
  ...
  async completeToken(tokenId: string) {
    ...
    if (updatedToken.servedAt && updatedToken.completedAt) {
      ...
      let newAvg = diffMins;
      if (currentAvgRaw) {
        newAvg = Math.max(1, Math.floor((parseInt(currentAvgRaw, 10) * 9 + diffMins) / 10)); // Exponential moving average over ~10 customers
      }
    ...
    this.webhooksService.triggerWebhooks(token.queue.tenantId, 'TOKEN_COMPLETED', token);
    return true;
  }
  ...
  async skipToken(tokenId: string) {
    ...
    this.webhooksService.triggerWebhooks(token.queue.tenantId, 'TOKEN_MISSED', token);
    return token;
  }
}
