...
export class TokenService {
  ...
  async joinQueue(queueId: string, customerName: string, phone?: string, otp?: string, formResponses?: any, language: string = 'en', scheduledFor?: string) {
    ...
    if (queue && queue.formConfig && Array.isArray(queue.formConfig)) {
      const purposeField = (queue.formConfig as any[]).find((f: any) => 
        (f.type === 'dropdown' || f.id === 'purpose') && f.label?.toLowerCase().includes('purpose')
      );
    ...
    if (phone) {
      if (isAppointment) {
        await this.notificationsService.sendWhatsAppMessage(
          phone,
          `Hello ${customerName}! Your appointment is scheduled for ${scheduledDate?.toLocaleString()}. Track your status here: http://localhost:3001/customer/status/${token.id}`
        );
      } else {
        await this.notificationsService.sendWhatsAppMessage(
          phone,
          `Hello ${customerName}! You have successfully joined the queue. You can track your live status here: http://localhost:3001/customer/status/${token.id}`
        );
      }
    }
  ...
  async advanceQueue(queueId: string) {
    ...
    if (currentlyServingId) {
      ...
      if (updatedToken.phone) {
        await this.whatsappService.requestFeedback(updatedToken.queue.tenantId, updatedToken.phone, updatedToken.language);
      }
    }
    ...
    if (upcomingTokenId) {
      const upcomingToken = await this.prisma.token.findUnique({ where: { id: upcomingTokenId } });
  ...
  async cancelToken(tokenId: string) {
    ...
    this.redisService.client.publish('queue_events', JSON.stringify({ type: 'TOKEN_CANCELLED', queueId: token.queueId, token: updatedToken }));
    
    const queue = await this.prisma.queue.findUnique({ where: { id: token.queueId } });
    if (queue) {
      this.webhooksService.triggerWebhooks(queue.tenantId, 'TOKEN_CANCELLED', updatedToken);
    }
  ...
  async transferToken(tokenId: string, nextQueueId: string) {
    ...
    if (servingTokenId === tokenId) {
      ...
      // Remove from old queue list if waiting
      await this.redisService.client.lrem(`queue:${token.queueId}:tokens`, 0, tokenId);
    }
    ...
    // Broadcast events
    this.redisService.client.publish('queue_events', JSON.stringify({ type: 'TOKEN_TRANSFERRED', oldQueueId: token.queueId, newQueueId: nextQueueId, token: updatedToken }));
    
    // Webhook Trigger
    const queue = await this.prisma.queue.findUnique({ where: { id: token.queueId } });
    if (queue) {
      this.webhooksService.triggerWebhooks(queue.tenantId, 'TOKEN_TRANSFERRED', updatedToken);
    }

    // Notify customer
    if (updatedToken.phone) {
      const newQueue = await this.prisma.queue.findUnique({ where: { id: nextQueueId } });
      await this.notificationsService.sendWhatsAppMessage(
        updatedToken.phone,
  ...
  async checkIn(tokenId: string) {
    ...
    this.redisService.client.publish('queue_events', JSON.stringify({ type: 'TOKEN_JOINED', queueId: token.queueId, token: updatedToken }));
